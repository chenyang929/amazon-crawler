...
class ParseProduct:
    ...
    # 产品标题
    def get_title(self):
        title_1 = self.sel.xpath('//span[@id="btAsinTitle"]/span/text()')
        title_2 = self.sel.xpath('//span[@id="productTitle"]/text()')
        title_3 = self.sel.xpath('//div[@id="title_feature_div"]/h1/text()')
        title_4 = self.sel.xpath('//div[@id="mnbaProductTitleAndYear"]/span/text()')
        title_5 = self.sel.xpath('//h1[starts-with(@class, "parseasinTitle")]/span/text()')
        title_6 = self.sel.xpath('//span[@id="ebooksProductTitle"]/text()')
        title_7 = self.sel.xpath('//div[contains(@id,"Title")]//h1/text()')
        return self._handle_str_xpath(title_1, title_2, title_3, title_4, title_5, title_6, title_7)
    ...
    def get_brand(self):
        ...
        return self._handle_str_xpath(brand_1, brand_2, brand_3, brand_4)
    ...
    def get_original_price(self):
        ...
        sign = domain['sign']
        original_price = self.sel.xpath('//div[@id="price"]//span[contains(@class, "a-text-strike")]/text()')
        if original_price:
    ...
    def get_price_maxprice(self):
        ...
        currency = domain['currency']
        sign = domain['sign']
        price_0 = self.sel.xpath('//span[contains(@id, "priceblock_pospromoprice")]/text()')
        ...
        price_8 = self.sel.xpath('//span[contains(@class, "a-color-base")]//span[contains(@class, "a-color-price")]/text()')
        if price_0:
            price = ''.join(price_0).replace(sign, '').replace(currency, '').replace(' ', '').replace('\xa0', '')
            if 'from' not in price:
                ...
                price = 0
            max_price = 0
        elif price_1 and price_1[0].strip():
            ...
            price = price_8[0].strip().replace(sign, '').replace(currency, '').replace(' ', '').replace('\xa0', '')
            max_price = 0
        else:
            price = 0
            max_price = 0
    ...
    def get_description(self):
        ...
        return self._handle_str_xpath(description_1, description_2)
